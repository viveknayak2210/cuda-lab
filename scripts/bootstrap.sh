...
# just prove we have a working GPU + compiler. The profiler (ncu) is installed
# lazily by run.sh, only when you actually profile, so boot stays ~10s.
set -uo pipefail
T0=$SECONDS
...
echo "next:  ./scripts/run.sh          (fast loop)"
echo "       PROFILE=1 ./scripts/run.sh (also grab an ncu report)"
